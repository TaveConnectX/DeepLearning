...
class RandomReplayBuffer:
    ...
    def sample(self):
        ...
        if self.use_conv:
            ...
            s_batch = torch.stack([s1 for (s1,*a,r,s2,d) in minibatch]).unsqueeze(1).to(self.device)
            s_prime_batch = torch.stack([s2 for (s1,*a,r,s2,d) in minibatch]).unsqueeze(1).to(self.device)
        ...
        a_batch = torch.Tensor([a[0] for (s1,*a,r,s2,d) in minibatch]).to(self.device)
        if self.use_minimax:
            b_batch = torch.Tensor([a[1] for (s1,*a,r,s2,d) in minibatch]).to(self.device)
            a_batch = 7*a_batch + b_batch
        r_batch = torch.Tensor([r for (s1,*a,r,s2,d) in minibatch]).to(self.device)
        d_batch = torch.Tensor([d for (s1,*a,r,s2,d) in minibatch]).to(self.device)
